...
> **Abstract.** 随机数是安全系统的基础需求,但可靠的物理随机源并不总是能在普通设备上方便获得。本论文研究一个具体问题:在只被动观察市场数据的前提下,公开的加密货币交易数据经过统计处理和标准密码学调理后,能否为强密码生成提供可用 entropy?加密货币市场全天连续交易,并产生高频公开数据,因此适合作为这一问题的测试对象。主要困难在于,原始价格变动并不独立:市场微观结构会使相邻价格变动符号强相关,所以这些数据不能被直接当作随机比特使用。
>
> 本论文构建并评估一条从市场价格到密码输出的处理流水线。价格变化先被编码为比特,再按不同时间口径聚合,随后通过统计检验筛选,并把多个资产的输出组合起来。通过这些步骤后,通过验证的比特流再进入标准密码学调理过程,用于生成最终密码。实验数据覆盖 5 个高流动性 USDT 现货交易对,时间跨度为 15 个月。
>
> 实验结果给出一个有条件的肯定答案。原始逐笔价格变化在所有月份中都未能通过统计检查。按 1 秒时间窗口聚合后,大部分短程依赖被去除,但单个资产的输出仍保留一部分更高阶结构。多资产组合进一步改善这些残留:三个配置在留出的验证月份上保持稳定,而一个四资产配置因为偏差漂移到校准范围之外而失败。最终 prototype 使用稳定配置生成 16-character passwords;按 search-space entropy 口径,每个密码约有 98 bits 强度——远高于普通用户自选密码常见的约 30 bits 量级,也超过本文采用的 80-bit 强随机密码工作基准。因此,本文结论是有限但实用的:在 passive statistical setting 与 prototype-level evaluation 下,公开加密货币数据经过聚合、验证与密码学调理后,可以作为密码生成的 entropy input。
